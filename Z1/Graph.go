	...
	for i := 0; i < N; i++ {
		for j := 0; j < K; j++ {
			...
			if i+powers[j] <= N && H(i)+1 == H(i+powers[j]) {
				graph.Graph_addEdge(i, i+powers[j], getCapacity(i, i+powers[j], K))
...
func (graph *Graph) Graph_copyForDynic() *Graph {
	...
	for _, n := range graph.nodes {
		for _, x := range n.neighbours {
			copy.Graph_addEdge(x, n.name, 0)
		}
	}
